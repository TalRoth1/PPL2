...
import {isClosure, makeClosure, Closure, Value, makeDictValue, isDictValue, isSymbolSExp} from "./L32-value";
                                    ...
                                    isDictExp(exp) ? evalDictExp(exp, env) :
                                        isAppExp(exp) ? bind(L32applicativeEval(exp.rator, env), (rator: Value) =>
                                            ...
                                            isLetExp(exp) ? makeFailure('"let" not supported (yet)') :
                                                    makeFailure(`Unexpected expression: ${format(exp)}`);
    ...
    isPrimOp(proc) ? applyPrimitive(proc, args) :
        isClosure(proc) ? applyClosure(proc, args, env) :
            isDictValue(proc) && args.length === 1 && isSymbolSExp(args[0]) ?
                (proc.val.hasOwnProperty(args[0].val) ? makeOk(proc.val[args[0].val]) : makeFailure("Error: Key not found")) :
...
const evalDictExp = (exp: DictExp, env: Env): Result<Value> =>
    bind(
        mapResult(([key, val] : [string, CExp]) => bind(L32applicativeEval(val, env),
            (evaluated: Value) =>
                makeOk([key, evaluated] as [string, Value])
            ), exp.pairs),
        (newPairs : List<[string, Value]>) => {
            const result: { [key: string]: Value } = {};
            newPairs.forEach(([key, val] : [string, Value]) => {
                result[key] = val;
            });
            return makeOk(makeDictValue(result));
        }
    );
